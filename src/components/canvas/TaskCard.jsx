    ...
    render() {
        ...
        return (
            <li className="list-group-item d-flex flex-row justify-content-start align-itens-center">
                ...
                className="m-2 btn btn-dark-blue btn-sm align-self-end" onClick={() => this.props.addTaskToMilestone(this.props.task)}>Add task</button>}
                {this.props.removeBtn && <button 
                className='m-2 btn btn-danger btn-sm align-self-end' onClick={() => this.props.removeTaskFromMilestone(this.props.task)}>Remove task</button>}
            </li>
        )
    }
}
